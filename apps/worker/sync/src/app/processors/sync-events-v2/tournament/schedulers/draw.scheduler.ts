...
export class DrawTournamentScheduler {
  private readonly logger = new Logger(DrawTournamentScheduler.name);
  ...
  @Process(Sync.ScheduleSyncTournamentDraw)
  async ScheduleSyncTournamentDraw(
    ...
    const executor = await this._syncQueue.add(Sync.ProcessSyncTournamentDraw, {
      transactionId,
      ...job.data,
    });

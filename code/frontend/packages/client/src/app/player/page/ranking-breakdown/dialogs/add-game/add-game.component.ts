...
import { Guid } from 'app/_shared/services/guid';
...
export class AddGameComponent implements OnInit {
  ...
  constructor(
    ...
    @Inject(MAT_DIALOG_DATA) public data: { playerId: string; type: 'single' | 'double' | 'mix'; system: RankingSystem }
  ) {
    this.fragment = gql`
  ...
  onConfirm(): void {
    ...
    const team1Points = this.p2t1Level.value
      ? (this._getWinningPoints(this.p1t1Level.value) + this._getWinningPoints(this.p2t1Level.value)) / 2
      : this._getWinningPoints(this.p1t1Level.value);

    const team2Points = this.p2t2Level.value
      ? (this._getWinningPoints(this.p1t2Level.value) + this._getWinningPoints(this.p2t2Level.value)) / 2
  ...
  private _getWinningPoints(level: number): number {
    const index = this.data!.system!.pointsWhenWinningAgainst!.length - level;
    return this.data!.system!.pointsWhenWinningAgainst![index];
  }
}

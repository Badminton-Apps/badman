...
import { combineLatest, combineLatestAll, filter } from 'rxjs';
...
export class CurriculumComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.offenseTypeControl = this._formBuilder.control('', [Validators.required]);
    ...
    combineLatest([
      ...
      .pipe(filter(([gameType, offenseType, place]) => !!gameType && !!offenseType && !!place))
      .subscribe(([gameType, offenseType, place]) => {
        this.actions = ['Aanvallende lift rechtdoor en cross', gameType, offenseType, place];
      });

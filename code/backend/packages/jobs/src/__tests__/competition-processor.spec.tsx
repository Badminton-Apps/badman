...
describe('competition processor', () => {
  beforeAll(async () => {
    new DataBaseHandler({
      dialect: 'sqlite',
      storage: ':memory:'
  ...
  describe('CompetitionSyncPointProcessor', () => {
    let processor: CompetitionSyncPointProcessor;

    beforeEach(() => {
      processor = new CompetitionSyncPointProcessor(null);
    });
    ...
    describe('Single', () => {
      it('type: S - Won: 1 - LevelDiff: 0', async () => {
        ...
        const event = await EventCompetitionBuilder.Create('738e888f-562d-49ec-9ace-5dcce8aafe0f')
          .WithName('event1')
          .WithSubEvent(
            SubEventCompetitionBuilder.Create()
              .WithGroup(group)
              .WithName('sub event 1')
              .WithDraw(
                DrawCompetitionBuilder.Create()
                  .WithName('draw1')
                  .WithEnouncter(
                    EncounterCompetitionBuilder.Create()
                      .WithDate(new Date('2021-10-15'))
                      .WithGame(game1)
                  )
              )
      ...
      it('type: S - Won: 2 - LevelDiff: 0', async () => {
        ...
        // Create event and assign game
        const event = await EventCompetitionBuilder.Create()
          .WithName('event1')
          .WithSubEvent(
            SubEventCompetitionBuilder.Create()
              .WithGroup(group)

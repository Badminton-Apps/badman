...
import { UseForTeamName } from '@badvlasim/shared/models/enums/useForTeams.enum';
...
(async () => {
  ...
  async function fixClubs() {
    const club1 = await Club.findByPk( '9ceaeed9-e8c9-42a7-b2c5-1982d1b50839', {transaction});
    club1.name = 'Oostendse';
    club1.useForTeamName =  UseForTeamName.ABBREVIATION;
    club1.abbreviation = 'OBTC';
    await club1.save({ transaction });

    const club2 = await Club.findByPk( 'd58e0d95-e3b2-4403-8222-d2dbdd42c06f', {transaction});
    club2.useForTeamName =  UseForTeamName.ABBREVIATION;
    club2.abbreviation = 'IZBA';
    await club2.save({ transaction });


    const club3 = await Club.findByPk( '9dd9f7f3-fe23-4d2c-a004-6f51fca20629', {transaction});
    club3.fullName = 'BC Oostrozebeke';
    club3.useForTeamName =  UseForTeamName.FULL_NAME;
    await club3.save({ transaction });


    const club4 = await Club.findByPk( '26147cfb-a5c1-4ac4-822f-b4d797c91b41', {transaction});
    club4.name = 'Zuid-West';
    await club4.save({ transaction });


    const club5 = await Club.findByPk( '60bbc3ab-f4f0-45f9-8a25-a331bbb30291', {transaction});
    club5.fullName = 'BC De Voskes';
    club5.useForTeamName =  UseForTeamName.FULL_NAME;
    await club5.save({ transaction });


    const club6 = await Club.findByPk( '2e1ebcf0-de7b-4b82-ac8e-37a9bafe54e9', {transaction});
    ...
    const club35 = await Club.findByPk( '52cd3b5b-ebf0-45c0-a3a6-94b928fc1ec3', {transaction});
    club35.name = 'Lede';
    club35.useForTeamName =  UseForTeamName.NAME; 
    await club35.save({ transaction });

    const club36 = await Club.findByPk( '3ffe1eea-b118-42d2-8426-14942bdc7c09', {transaction});
    club36.name = 'Evergem';
    club36.useForTeamName =  UseForTeamName.NAME;
    await club36.save({ transaction });

    const club37 = await Club.findByPk( '1d928e29-4f2c-4695-bdf5-898363c07290', {transaction});
    club37.name = 'Flee Shuttle';
    club37.useForTeamName =  UseForTeamName.NAME;
    await club37.save({ transaction });

    const club38 = await Club.findByPk( 'e60a7152-9f6f-4ee0-bc6e-43e48e8db99a', {transaction});
    club38.name = 'De Wallabies';
    club38.useForTeamName =  UseForTeamName.NAME;
    await club38.save({ transaction });

    const club39 = await Club.findByPk( '2bdf111e-14bd-4f29-8680-c6802d6cbedf', {transaction});
    club39.name = 'Danlie';
    club39.useForTeamName =  UseForTeamName.NAME;
    await club39.save({ transaction });

    const club40 = await Club.findByPk( '76952221-7b55-49ec-b22f-db1674b5733f', {transaction});
    club40.name = 'Neerpelt';
    club40.useForTeamName =  UseForTeamName.NAME;
    await club40.save({ transaction });

    const club41 = await Club.findByPk( '17ebb48d-eaa6-4c4f-b34b-f29cf2aa4496', {transaction});
    club41.useForTeamName =  UseForTeamName.FULL_NAME;
    await club41.save({ transaction });

    const club42 = await Club.findByPk( 'e2bfad97-ae6a-42d6-8bc4-b0f819ad05a1', {transaction});
    club42.name =  'Ieper';
    club42.useForTeamName =  UseForTeamName.NAME;
    await club42.save({ transaction });

    const club43 = await Club.findByPk( '126d87c2-cfe5-468f-ab14-6276631ea253', {transaction});
    club43.name =  'PSV Brugge';
    club43.useForTeamName =  UseForTeamName.NAME;
    await club43.save({ transaction });

    const club44 = await Club.findByPk( 'fee975cc-f91c-4bac-8eee-646f4562d19c', {transaction});

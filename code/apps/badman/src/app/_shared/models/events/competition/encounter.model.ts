...
export class CompetitionEncounter {
  ...
  constructor(args: Partial<CompetitionEncounter>) {
    ...
    this.games = args?.games
      ...
      ?.sort((a, b) => {
        const aSort =
          (a.order ?? null) != null ? a.order : (a.round ?? null) != null ? parseInt(a.round!.replace('R', ''), 10) : 0;
        const bSort =
          (b.order ?? null) != null ? b.order : (b.round ?? null) != null ? parseInt(b.round!.replace('R', ''), 10) : 0;

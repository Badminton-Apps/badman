...
- **Ranking writes go through `RankingPlaceWriterService`**: all `RankingPlace` and `RankingLastPlace` writes MUST use `RankingPlaceWriterService` from `@badman/backend-database` (feature 037-ranking-write-protection). Never call `RankingPlace.bulkCreate / create / upsert / findOrCreate` directly — the root eslint config enforces this. Contract: [`specs/037-ranking-write-protection/contracts/ranking-place-writer.md`](specs/037-ranking-write-protection/contracts/ranking-place-writer.md).
...
[specs/037-ranking-write-protection/plan.md](specs/037-ranking-write-protection/plan.md)

...
**Related (third Stage A CI round): Nx CLI cannot be invoked as N concurrent processes.** Because the Stage A scripts wrap `nx <target> <proj>`, `turbo run … --concurrency=3` spawns several `nx` CLIs at once, which race on Nx project-graph computation and intermittently fail with the same `no name provided: scripts` error. A single `nx <target>` invocation always succeeds. Fix: run the Stage A double-run with **`--concurrency=1`** (serialize the Nx calls) and drop `^build` from the Stage A `turbo.json` `lint`/`test` tasks (Nx already resolves build deps internally, so `^build` only multiplied the concurrent `nx` invocations). Turborepo's real parallelism + `^build` ordering return in Stage B once the package scripts call `jest`/`eslint`/`tsc` directly with no Nx underneath. Net Stage A lesson: every one of these gotchas (D12b/D12c + this) stems from `nx` sitting under the package scripts — all evaporate when Nx is removed in Stage B.

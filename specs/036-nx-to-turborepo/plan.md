...
**This migration swaps a constitution-load-bearing stack item (Monorepo: Nx). The required MAJOR amendment has landed on this branch (constitution v2.0.0); the only residual sync is AGENTS.md, carried by the Phase 2 PR (FR-016). See Constitution Check + Complexity Tracking.**
...
| ----------------------------------------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Tech Stack: "Monorepo: Turborepo + pnpm workspaces" (v2.0.0)**              | ✅ Resolved | Stack entry was amended from Nx → Turborepo+pnpm in constitution v2.0.0. `@badman/<name>` import naming is **preserved**. No outstanding violation.    |
| **Tech Stack: "Tests: Jest … `turbo run test`" (v2.0.0)**                     | ✅ Resolved | Jest retained; the invocation wording was updated to `turbo run test` in the amendment.                                                                |
| **Tech Stack: "Formatting: Prettier"**                                        | ✅          | Unchanged.                                                                                                                                             |
| **Dev Workflow: `turbo run …` commands (v2.0.0)**                             | ✅ Resolved | Default dev + affected-check commands updated to `turbo run …` in the amendment. AGENTS.md command-surface rewrite still pending (FR-016, Phase 2 PR). |
| **Principle V — Frontend Lives in a Separate Repository (v2.0.0)**            | ✅ Resolved | Principle V redefined: legacy Angular removed, repo is backend-only. The FE deletion (FR-014) now complies with the amended principle.                 |

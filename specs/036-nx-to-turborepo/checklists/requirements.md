...
- Post-plan, the official Turborepo "Migrating from Nx" guide was reviewed. It revised one earlier decision (cutover: single big-bang → **phased hybrid** — Phase 1 incremental coexistence with CI double-run verification, Phase 2 atomic structural cutover; FR-017) and added two requirements (FR-021 per-package dependency ownership, FR-022 `.turbo` gitignore; SC-014). Command equivalents corrected (`--parallel`→`--concurrency`). The constitution was amended to v2.0.0 (Nx→Turborepo+pnpm stack swap; Principle V redefined backend-only) to clear the plan's merge gate.

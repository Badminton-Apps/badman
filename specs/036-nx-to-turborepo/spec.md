...
- Q: Cutover strategy? → A: **Single cutover PR** — one branch flips Nx→Turborepo atomically. **(SUPERSEDED — see Session 2026-06-09 #2 below.)**

### Session 2026-06-09 #2 (post official-guide review)

- Q: After reading the official Turborepo "Migrating from Nx" guide (which _encourages_ incremental coexistence + double-run verification), keep single-cutover or phase it? → A: **Phased hybrid.** **Phase 1 (incremental, coexisting):** add `turbo.json` + per-package `build`/`test`/`lint` scripts _over the existing Nx project structure_ (apps/libs unchanged, `tsconfig.paths` intact, still npm), with the per-package scripts initially delegating to the real builders; run Turborepo and Nx side-by-side and **double-run** (`turbo run lint test` AND `nx affected -t lint test`) in CI to verify graph/cache/affected parity before trusting Turborepo. **Phase 2 (atomic structural cutover):** in one PR, perform the `libs/`→`packages/` move, drop `tsconfig.paths` for `workspace:*` + `exports`, switch app builds to `nest build`, adopt pnpm, rewrite CI to Turborepo-only, and remove Nx. The `tsconfig.paths`↔`workspace:*` resolution conflict only exists during Phase 2's structural change, so that part stays atomic; the task-runner adoption is de-risked incrementally per the official guide.
- Q: Where do dependencies live? → A: Follow the guide — **move dependencies into the packages/apps that use them** (Nx installs everything at the repo root; Turborepo prefers per-package deps for better pruning + cache-hit ratios). Internal `@badman/*` deps become `workspace:*`; external deps (NestJS, Sequelize, etc.) migrate from the root `package.json` to each consumer. This MAY be done progressively but MUST be complete by end of Phase 2.
...
- **Migration phasing**: Phase 1 deliberately runs Nx and Turborepo together (FR-017) — the risk is _drift_ between the two runners' results, which the mandatory CI double-run is designed to catch before Nx is removed. Phase 2's structural change (paths→workspace-deps, libs→packages, pnpm) is the only genuinely atomic part and MUST NOT be split into partial commits that leave the repo with mixed resolution on `develop`.
...
- **FR-005**: The `@badman/*` imports MUST continue to resolve for build, test, lint, and IDE tooling without requiring changes to import statements in consuming code. Resolution moves from Nx `tsconfig.paths` to native pnpm workspace dependencies (each consumer declares `"@badman/<name>": "workspace:*"`) plus each package's `exports` map; the import specifiers themselves are unchanged because each package's name equals its current alias.
...
- **FR-017**: The migration MUST proceed in two phases, each merged to `develop` only when green. **Phase 1 (incremental coexistence)**: introduce `turbo.json` + per-package `build`/`test`/`lint` scripts over the _existing_ Nx structure (apps/libs unchanged, `tsconfig.paths` intact, npm retained); Nx and Turborepo run side-by-side and CI MUST **double-run** Turborepo and Nx for the same tasks to verify graph/cache/affected parity. **Phase 2 (atomic structural cutover)**: in a single PR, perform the `libs/`→`packages/` move, drop `tsconfig.paths` in favor of `workspace:*` + `exports`, switch apps to `nest build`, adopt pnpm, rewrite CI to Turborepo-only, and remove Nx. The `tsconfig.paths`↔`workspace:*` resolution conflict MUST NOT exist simultaneously — that conflict is confined to Phase 2, which is therefore atomic; Phase 1 keeps both runners valid because the per-package scripts wrap the existing builders.
- **FR-020**: The repository MUST use pnpm as its package manager and workspace provider: a `pnpm-workspace.yaml` declaring `apps/*` and `packages/*`, `workspace:*` internal-dependency syntax, a pnpm lockfile (replacing `package-lock.json`), and CI plus the Render build command updated to pnpm. No npm `package-lock.json` or npm-workspaces config remains. (The repo builds no app Docker image — `docker-compose.dev.yml` only pulls local infra services (Postgres/Redis/pgAdmin) and is unaffected by the package-manager switch.) (Phase 2.)
- **FR-021**: Dependencies MUST be located in the package/app that uses them rather than only at the repo root (per the official Turborepo guide — improves dependency pruning and cache-hit ratio). Internal `@badman/*` dependencies become `workspace:*`; external runtime/dev dependencies migrate from the root `package.json` into each consuming package's `package.json`. This MUST be complete by end of Phase 2.
- **FR-022**: The `.turbo` cache directory MUST be added to `.gitignore`.
...
- **SC-013**: Phase 1 lands with CI double-running Turborepo and Nx and reporting parity (same affected set, same pass/fail) for at least one real PR before Phase 2 begins; Phase 2's structural cutover merges atomically, after which `develop` is green with Turborepo only and no mixed-resolution intermediate state.
- **SC-014**: After Phase 2, every package declares its own direct dependencies (internal as `workspace:*`, external in its `package.json`); the root `package.json` carries only workspace-root tooling (turbo, release-please, formatters), not application/library runtime deps.
...
- The migration follows the official Turborepo "Migrating from Nx" guide's phased approach: Phase 1 runs Nx and Turborepo side-by-side with CI double-run verification; Phase 2 is the atomic structural cutover (paths→workspace-deps, libs→packages, pnpm, Nx removal). Dependencies move from the repo root into the packages that use them by end of Phase 2.

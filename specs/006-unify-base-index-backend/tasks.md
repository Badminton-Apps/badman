...
- [ ] T006 [P] Create `libs/backend/graphql/src/resolvers/event/competition/calculate-index/calculate-index.result.ts` with code-first `@ObjectType` classes: `CalculateIndexContributingPlayer`, `CalculateIndexError`, `CalculateIndexResult`. Add a barrel `libs/backend/graphql/src/resolvers/event/competition/calculate-index/index.ts` re-exporting input + result + (forthcoming) resolver.
...
- [ ] T010 [US2] In `index-calculation.service.ts`, implement per-player component fill: for each input player, the resolved `single`/`double`/`mix` is `caller-supplied ?? snapshot ?? rankingSystem.amountOfLevels`. The fully-resolved player object also carries `gender` (from input or, when absent, from the `Player` table — single batched lookup per input set).
- [ ] T011 [US2] In `index-calculation.service.ts`, implement gender resolution: when `players[*].gender` is undefined, batch-load `Player.findAll({ where: { id: [...] }, attributes: ['id','gender'] })` and merge into the resolved player set. When a player ID has no `Player` row, surface `IndexCalculationFailure` with code `PLAYER_NOT_FOUND` and `playerIds: [<id>]` for that single input only — do not fail siblings.
...
- [ ] T020 [US1] In `calculate-index.resolver.ts`, add resolver-level pre-validation that throws `BadRequestException` for the whole batch on: (a) empty `inputs` array, (b) any duplicate `key` within the batch, (c) malformed UUID in `rankingSystemId`/`subEventCompetitionId`/player `id` (use existing `IsUUID` helper from `@badman/utils` if present, otherwise inline `class-validator` validation), (d) `season` outside `[1990, currentYear + 1]`.
- [ ] T021 [US1] In `calculate-index.resolver.ts`, enforce authentication: if `user` is null OR `user.hasAnyPermission` is the anonymous stub (always-`false`), throw `UnauthorizedException` per FR-006a. Reference: existing resolvers that use `@AllowAnonymous()` show the inverse pattern.
- [ ] T022 [US1] In `calculate-index.resolver.ts`, fold the service's `IndexCalculationSuccess | IndexCalculationFailure` discriminated union into the GraphQL `CalculateIndexResult` shape: when Success, populate `index`, `contributingPlayers`, `missingPlayerCount`, leave `error` null; when Failure, leave numeric/list fields null and populate `error: { code, message, playerIds? }`. Preserve input order.
...
- [ ] T024 [P] [US1] Create `libs/backend/graphql/src/resolvers/event/competition/calculate-index/calculate-index.resolver.spec.ts` per Constitution Principle IV: `Test.createTestingModule` with the resolver + a mocked `IndexCalculationService` (jest.fn for `calculate`); fake `Player` with `hasAnyPermission` jest.fn(); `afterEach(jest.restoreAllMocks)`. Cases: (a) authenticated success — service returns mixed Success/Failure list, resolver returns the correctly-folded GraphQL shape; (b) anonymous user → `UnauthorizedException`; (c) empty `inputs` → `BadRequestException`; (d) duplicate `key` within batch → `BadRequestException`; (e) malformed UUID in any field → `BadRequestException`; (f) `season` out of range → `BadRequestException`.

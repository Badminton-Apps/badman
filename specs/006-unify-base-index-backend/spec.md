...
- **FR-001**: The platform MUST expose a single backend operation that, given (a) a team type (M / F / MX), (b) a list of player identifiers, (c) the season, and (d) an optional sub-event ID, returns the index value, the list of players that contributed to it, and the count of missing players that incurred a penalty. The service resolves all per-player data (gender, ranking components) exclusively from the database — callers never supply ranking values or gender. The service always uses the primary ranking system; callers do not specify a ranking system ID.

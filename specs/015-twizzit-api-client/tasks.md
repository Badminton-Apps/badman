...
**Tests**: REQUIRED for this feature (FR-017, FR-018, FR-019, SC-002, SC-003, SC-004). Every endpoint function gets unit + fixture tests; an opt-in live-mode suite mirrors them.
...
- [ ] T006 Add `zod` as a direct dependency: `npm install zod` at the repo root (Nx hoists). Confirm `package.json` records `zod` under `dependencies`.
...
**Purpose**: Cross-cutting infrastructure every user story depends on: logger, error classes, redaction, HTTP wrapper, pagination helper, federation seam, import-boundary guard, shared schemas.
...
- [ ] T010 [P] Create the secret-redaction utility at `libs/integrations/twizzit-client/src/redact.ts`. Public surface: `redact(value: unknown, secrets: ReadonlyArray<string>): unknown` (deep-walks strings/objects/arrays, replaces any substring matching one of `secrets` with `[REDACTED]`), `redactExcerpt(body: string, secrets: ReadonlyArray<string>, maxLen?: number): string`. Plus a dedicated test `libs/integrations/twizzit-client/test/redact.spec.ts` covering: token in nested object, password in JSON-stringified body, `Authorization: Bearer …` header value redacted, max-length truncation works, primitive non-string values pass through unchanged.
- [ ] T011 [P] Create all six error classes at `libs/integrations/twizzit-client/src/errors.ts` matching [contracts/errors.ts](contracts/errors.ts) exactly: `TwizzitAuthError`, `TwizzitValidationError`, `TwizzitNetworkError`, `TwizzitRateLimitError`, `TwizzitServerError`, `TwizzitClientError`. Each extends `Error`, has the `kind` discriminant as a readonly literal, and constructs every field through `redact()` (so token/password can never sneak in). Export `isTwizzitError(e: unknown): e is TwizzitError` type-guard.
...
- [ ] T013 Create the HTTP wrapper at `libs/integrations/twizzit-client/src/http.ts`: function `httpRequest(opts: { url, method, headers, body?, signal? }, redactSecrets: ReadonlyArray<string>, logger: Logger): Promise<{ status, body, headers }>`. Internals use Node global `fetch`; on `ECONNRESET`/timeout/DNS errors throws `TwizzitNetworkError`; on 5xx returns the response object (caller classifies); body is read once as text, JSON-parsed lazily, excerpts always go through `redactExcerpt`. (Depends on T010, T011, T009.)
...
- [ ] T015 [P] Create the federation seam at `libs/integrations/twizzit-client/src/seam.ts`: TypeScript interfaces `FederationOrganization`, `FederationContactSource` per research.md R9 and [contracts/client.ts](contracts/client.ts). No implementation in this file — just the types.
...
- [ ] T017 [P] [US1] Add fixture `libs/integrations/twizzit-client/test/__fixtures__/authenticate.200.json` based on the live response captured 2026-05-12 (token redacted/synthetic, `created-on`/`valid-till` kept).
...
- [ ] T019 [US1] Create `libs/integrations/twizzit-client/test/client.auth.spec.ts` with describe blocks for: (a) authenticate happy path returns void and caches token; (b) authenticate 401 → `TwizzitAuthError`; (c) authenticate network error → `TwizzitNetworkError`; (d) authenticate response missing `valid-till` → `TwizzitValidationError`; (e) credential redaction — pass deliberately-leaky username/password and assert neither appears in any thrown error's `message`, `bodyExcerpt`, or any logged `meta` (logger captured via a test-double `Logger`); (f) getOrganizations happy path returns the parsed array; (g) getOrganizations called before authenticate → either auto-authenticates (preferred) or throws `TwizzitClientError` with `subkind: "missing-organization-id"`. Mock HTTP via injecting a fake `fetch` implementation through the lib's testability seam (`TwizzitClient` constructor accepts `{ fetch?: typeof fetch }` in the test build) OR by wrapping the http module — pick the simplest. (Depends on T013, T011, T012.)
...
- [ ] T023 [US1] Create the authenticate endpoint at `libs/integrations/twizzit-client/src/endpoints/authenticate.ts`: `async function authenticate(http, credentials, logger): Promise<AuthenticateResponse>`. POSTs to `${baseUrl}/authenticate` with JSON body `{ username, password }`, classifies 401/403 → `TwizzitAuthError`, parses response with `AuthenticateResponseSchema`, redacts username/password from any thrown error. (Depends on T013, T021, T011.)
...
- [ ] T039 [US2] Wire the four methods into `TwizzitClient` at `libs/integrations/twizzit-client/src/client.ts`: `getContacts`, `getMemberships`, `getMembershipTypes`, `getExtraFields`. Each method: ensure auth (refresh if needed), ensure organizationId resolved (call `getOrganizations()` if `config.organizationId` is undefined and `this.organizationId` is unset — pick the first entry, log a `warn` if more than one). Implement the same 401-retry-once wrapper as on US1 methods. Add the four `fetchXxx` aliases that satisfy `FederationContactSource`. (Depends on T025, T035, T036, T037, T038.)
- [ ] T040 [P] [US2] Update `libs/integrations/twizzit-client/src/index.ts` to export every schema, every inferred type, `getMemberId`, `FederationContactSource`, all six error classes, `isTwizzitError`, `noopLogger`, `Logger`, `TwizzitClient`, and all config types.
...
## Phase 5: User Story 3 — Structured, redacting error handling (Priority: P1)

**Goal**: All error variants are exercised, the redaction invariant is regression-tested, the discriminated union is the public contract.

**Independent Test**: Each of the six `TwizzitError` variants can be deliberately induced in a unit test against a mocked HTTP layer and asserted by `kind`. A redaction grep over emitted test output never finds the bearer token, password, or full `Authorization` header value.

### Tests for User Story 3

- [ ] T042 [P] [US3] Create `libs/integrations/twizzit-client/test/client.errors.spec.ts` with one describe-block per variant: (a) `TwizzitAuthError` — already covered in T019/T020 but add the case where `/organizations` returns 403 even with a fresh token; (b) `TwizzitValidationError` — synthetic mutated `organizations.broken.json` (one entry missing `name`) → assert `path` mentions index + `name`; (c) `TwizzitNetworkError` — mock fetch to reject with `Error('ECONNRESET')` → assert `code === 'ECONNRESET'`; (d) `TwizzitRateLimitError` — first GET returns 429 with `Retry-After: 1`, second returns 429 with `Retry-After: 2`, third returns 429; with `maxRateLimitRetries: 2` assert throw on third with `retryAfterMs: 2000` and `attempts: 3`; (e) `TwizzitServerError` — 503 with body `<html>down</html>` → assert `bodyExcerpt` redacted, no HTML in clear text leaks token (n/a here, but the redact pipeline runs); (f) `TwizzitClientError` — 422 with body `{"error":"bad limit"}` → assert `status: 422`, `subkind: undefined`; (g) `TwizzitClientError` subkind `max-pages-exceeded` — already in T030, reference it here for traceability.
- [ ] T043 [US3] Extend `redact.spec.ts` (existing from T010) with end-to-end pipeline tests: spin up a `TwizzitClient` with a deliberately silly password like `"hunter2_LEAK_THIS"` and a deliberately silly username; trigger each error variant against a fake fetch; serialise each thrown error via `JSON.stringify(err, Object.getOwnPropertyNames(err))` and grep — assert the password string and bearer token string never appear in the serialised output. Repeat the assertion for every `logger.warn` and `logger.error` call captured by the test-double logger. This is the FR-016 / SC-004 regression gate.
...
(No production-code tasks here — error classes (T011), redaction (T010), and the per-status classification inside `http.ts` (T013) already shipped under foundational + US1/US2. US3 is realized through coverage.)

- [ ] T045 [US3] If the test work in T042/T043/T044 exposes any error-field that is constructed without going through `redact()`, fix it in `src/errors.ts` and `src/http.ts`. Document the fix as a follow-up bullet in [research.md](research.md). (Implementation task only if needed.)

**Checkpoint**: Every error variant has a regression test; the redaction invariant has an end-to-end test; the discriminated union is the documented public contract.
...
- [ ] T046 [P] [US4] Audit every `*.spec.ts` (non-`.live`) in `libs/integrations/twizzit-client/test/` to confirm zero direct `fetch` / `node-fetch` / network calls. Add a defensive `globalThis.fetch = () => { throw new Error('Real fetch attempted in offline test'); }` at the top of a Jest setup file `libs/integrations/twizzit-client/test/setup.offline.ts`, wired via `setupFilesAfterEach` in `jest.config.ts` *only* when `RUN_TWIZZIT_LIVE_TESTS` is unset.
...
Task: "T010 Create redact utility + test at libs/integrations/twizzit-client/src/redact.ts + test/redact.spec.ts"
...
Task: "T015 Create federation seam at libs/integrations/twizzit-client/src/seam.ts"

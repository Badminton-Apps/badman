...
- **Phase 1** — architect & detailed design for the read-only client layer (auth, request pattern, error taxonomy, federation-agnostic seam, validation strategy).
...
- **FR-005**: An ADR (or design doc under `docs/twizzit/`) MUST capture: (a) where the Twizzit-specific code lives in the monorepo, (b) the federation-agnostic seam (interface, not implementation), (c) the validation strategy (strict vs passthrough zod), (d) the pagination strategy per endpoint, (e) the error taxonomy.
...
- **FR-008**: The seam between "talk to Twizzit" and "reconcile to Badman" MUST be expressible as a TypeScript interface in this lib, so a non-Twizzit federation can later supply an alternative implementation (per N3.1, gap Q21).
...
- **FR-010**: Every response shape MUST be defined as a zod schema in the same lib. The TypeScript types consumed by callers MUST be inferred from those schemas, so the schema is the single source of truth for both validation and types.
...
- **FR-013**: On a `401` response mid-session, the client MUST re-authenticate once and retry the failing request; a second `401` MUST throw a typed auth error (per F1.4).
- **FR-014**: On a `429` response, the client MUST honour `Retry-After` and apply bounded exponential back-off; on exhaustion the client MUST throw a typed rate-limit error carrying `retryAfterMs` and the attempt count (per N1.1).
...
- **FR-016**: Neither the bearer token, the password, nor any `Authorization` header value MAY appear in any error message, log line emitted to the optional `Logger` (see Key Entities), fixture, or serialised output. Enforced by a dedicated test (per N4.1).
- **FR-017**: Every client function MUST have unit tests using a mocked HTTP layer that cover: happy-path parsing, schema failure, 401-then-success retry, 401-then-401 final failure, 429 honour-retry-after, 5xx surfacing, network error surfacing, credential redaction.
...
- **SC-004**: A code review of all error messages, log lines, and serialised fixtures produces zero hits when grepped for the literal `Authorization` header value, the password, or the bearer token. A dedicated automated test enforces this.
...
- The lib uses zod for schemas (decided here because the user explicitly named zod). HTTP client choice (`fetch`, `axios`, `undici`) is a Phase 1 ADR decision — the lib's public surface MUST NOT leak that choice.
...
- The federation-agnostic seam (FR-008) is shaped as an interface in the same lib for now and may move to a higher-level shared lib in a future phase when a second federation actually appears. This avoids speculative abstraction (per CLAUDE.md guidance: three similar lines beats a premature abstraction).

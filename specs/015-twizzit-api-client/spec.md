...
- **FR-017**: Every client function MUST have unit tests using a mocked HTTP layer (axios mock adapter, axios interceptor stub, or DI seam) that cover: happy-path parsing, schema failure, 401-then-success retry, 401-then-401 final failure, 429 honour-retry-after, 5xx surfacing, network error surfacing.

...
- **FR-021**: For list endpoints, pagination MUST be transparent: each list function loops Twizzit's offset-based pagination (`limit` + `offset` query parameters) internally and returns the validated list to the caller. Each list function MUST accept an optional `{ pageSize?: number; maxPages?: number }` argument. When `maxPages` is **unset** (the default for production sync), the loop fetches every page until the federation returns a short/empty page — this is the behaviour required to ingest a complete federation snapshot (e.g. ~160 000 BV contacts ÷ 50 per page ≈ 3200 pages). When `maxPages` is **set** by the caller (sampling / debugging), the loop stops after that many pages and returns the partial result; a `logger.warn` is emitted documenting the truncation. An internal `RUNAWAY_PAGE_LIMIT` (currently 100 000, not user-configurable) protects against pathological federation behaviour — only that ceiling can produce a `TwizzitClientError` with `subkind: "pagination-runaway"`. Multi-page behaviour MUST be exercised by a fixture test that captures at least two pages of response, plus a test asserting truncate-with-warning when `maxPages` is exceeded by the synthetic data. *(2026-05-13: previously `maxPages` defaulted to 2000 and threw `subkind: "max-pages-exceeded"` on overflow; that contradicted the actual sync requirement, where 160k contacts at pageSize 50 exceeds 2000 pages. Truncate-by-default is the correct semantic.)*

      ...
      this.teams()
        ...
        .subscribe(() => {
          ...
          for (const type of this.eventTypes) {
            ...
            subeventIds = subeventIds.concat(teamForms.map((team) => team.entry?.subEventId ?? ""));
          }
          ...
          // distinct event.type
          this.validTypes.set(Array.from(new Set(events.map((event) => event.type as LevelType))));
        });
    });
  }
}

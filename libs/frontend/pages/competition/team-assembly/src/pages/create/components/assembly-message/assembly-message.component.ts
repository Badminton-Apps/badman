...
export class AssemblyMessageComponent implements OnInit {
  ...
  private _getPlayers() {
    ...
    if (this.validation?.params?.['team1player1']) {
      const team1player1 = this.validation?.params?.[
        'team1player1'
      ] as ValidationPlayer;
    ...
    if (this.validation?.params?.['team1player2']) {
      const team1player2 = this.validation?.params?.[
        'team1player2'
      ] as ValidationPlayer;
    ...
    if (this.validation?.params?.['team2player1']) {
      const team2player1 = this.validation?.params?.[
        'team2player1'
      ] as ValidationPlayer;
    ...
    if (this.validation?.params?.['team2player2']) {
      const team2player2 = this.validation?.params?.[
        'team2player2'
      ] as ValidationPlayer;
  ...
  private _getIndex() {
    ...
    if (this.validation?.params?.['teamIndex']) {
      const teamIndex = this.validation?.params?.[
        'teamIndex'
      ] as ValidationPlayer;
    ...
    if (this.validation?.params?.['baseIndex']) {
      const baseIndex = this.validation?.params?.[
        'baseIndex'
      ] as ValidationPlayer;
    ...
    if (this.validation?.params?.['minIndex']) {
      const minIndex = this.validation?.params?.[
        'minIndex'
      ] as ValidationPlayer;
  ...
  private _getMaxLevel() {
    ...
    if (this.validation?.params?.['minLevel']) {
      const minLevel = this.validation?.params?.[
        'minLevel'
      ] as ValidationPlayer;
      params['minLevel'] = minLevel;
    }
    if (this.validation?.params?.['rankingType']) {
      const rankingType = this.validation?.params?.[
        'rankingType'
      ] as ValidationPlayer;

 ...
 */
export function parseResponse(data: string | unknown, parser: XMLParser, logger: Logger): unknown {
...
export function validateMany<TOut>(
  ...
  if (!result.success) {
    logger.error(`Visual API returned a malformed ${payloadKey} payload: ${result.error.message}`);
    throw new Error(`Invalid ${payloadKey} response from Visual API: ${result.error.message}`);
  }
    ...
    if (result.success) {
      ...
      if (onDrop) {
        try {
          ...
          logger.error(`[visual] onDrop callback threw while reporting ${payloadKey} drop:`, cbErr);
        }
      }
    }
  }

...
describeIfWindows("CpFileWriter integration (Windows-only)", () => {
  ...
  const templatePath = join(process.cwd(), "packages/backend-generator/assets/empty.cp");
  ...
  it("should generate a valid .cp file from a test payload", async () => {
    ...
    execSync(
      `node packages/backend-generator/dist/scripts/cp-file-writer.js ${payloadPath} --output ${outputPath}`,
      {
        env: { ...process.env },
        stdio: "pipe",
      }

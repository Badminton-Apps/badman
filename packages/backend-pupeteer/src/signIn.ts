...
export async function signIn(
  ...
  // LOGIN
  {
    ...
    try {
      ...
      const msg =
        loginButtonError instanceof Error ? loginButtonError.message : String(loginButtonError);
    ...
    const navigationPromise = targetPage
      ...
      .catch((error: any) => {
        ...
        if (error?.message?.includes("detached") || error?.message?.includes("Frame")) {
          logger?.warn(
            "Navigation frame detached, page may have already navigated:",
            error?.message
          );
          return null;
        }
    ...
    try {
      ...
      if (error?.message?.includes("detached") || error?.message?.includes("Frame")) {
        logger?.warn(
          "Navigation promise failed due to detached frame, continuing:",
          error?.message
        );
  ...
  }
  {
    ...
    const navigationPromise = targetPage
      ...
      .catch((error: any) => {
        ...
        if (error?.message?.includes("detached") || error?.message?.includes("Frame")) {
          logger?.warn(
            "Navigation frame detached during login, page may have already navigated:",
            error?.message
          );
          return null;
        }
    ...
    try {
      ...
      if (error?.message?.includes("detached") || error?.message?.includes("Frame")) {
        logger?.warn(
          "Login navigation promise failed due to detached frame, continuing:",
          error?.message
        );
        // Wait a bit for page to stabilize
        await new Promise((resolve) => setTimeout(resolve, 1000));

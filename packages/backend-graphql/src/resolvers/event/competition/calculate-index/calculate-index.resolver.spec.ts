...
import { INDEX_CALCULATION_FIXTURES, getIndexFromPlayers, SubEventTypeEnum } from "@badman/utils";
...
describe("CalculateIndexResolver", () => {
    ...
    mockService.calculate.mockResolvedValue([
      {
        ...
        key: "team-key-1",
        index: 64,
        contributingPlayers: [{ id: "p1", gender: "M", single: 8, double: 8, mix: 12 }],
  ...
  it("throws GraphQLError with PLAYER_NOT_FOUND code when service returns a failure", async () => {
    ...
    await expect(resolver.calculateIndex([validInput()], user)).rejects.toThrow(GraphQLError);

    await expect(resolver.calculateIndex([validInput()], user)).rejects.toMatchObject({
  ...
  it("throws GraphQLError BAD_USER_INPUT when season is below 1990", async () => {
    ...
    await expect(resolver.calculateIndex([validInput({ season: 1989 })], user)).rejects.toThrow(
      GraphQLError
    );
    await expect(

  ...
  imports: [
    BullModule.forRootAsync({
      ...
      useFactory: async (configService: ConfigService<ConfigType>) => {
        const lockDuration = configService.get<number>("QUEUE_LOCK_DURATION_MS") ?? 5 * 60 * 1000; // 5 minutes default
        const lockRenewTime =
          configService.get<number>("QUEUE_LOCK_RENEW_TIME_MS") ?? Math.floor(lockDuration / 2); // half of lock duration
        const maxStalledCount = configService.get<number>("QUEUE_MAX_STALLED_COUNT") ?? 3;
        const jobTimeout = configService.get<number>("QUEUE_JOB_TIMEOUT_MS") ?? 30 * 60 * 1000; // 30 min default: cap job runtime so stuck jobs don't block the queue

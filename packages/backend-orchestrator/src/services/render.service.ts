...
export class RenderService {
  ...
  async startService(service: Service) {
    ...
    this._logger.debug(
      `startService: fetching status for ${service.name} (renderId=${service.renderId})`
    );
  ...
  async suspendService(service: Service) {
    ...
    this._logger.debug(
      `suspendService: fetching status for ${service.name} (renderId=${service.renderId})`
    );
    const serviceData = await this.getService(service, false);
  ...
  public async getService(service: Service, setStatus = true) {
    if (!service.renderId) {
      this._logger.error(
        `getService: service "${service.name}" (id=${service.id}) has no renderId`
      );
      throw new Error(`No render id found for service ${service.name}`);
    }
